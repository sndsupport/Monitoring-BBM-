...
- **PIC CABANG read-only untuk aksi operasional (SUPERADMIN-only):** PIC tetap bisa input laporan harian baru & membuat jalur pengiriman, serta mengelola master Supir/Kendaraan/Kartu Flazz cabangnya sendiri dan melihat semua view cabangnya. Aksi operasional berikut dikunci **SUPERADMIN** di sisi server (`assertSuperadminOnly`) dan tombolnya disembunyikan untuk PIC: edit/hapus laporan harian, edit/hapus jalur pengiriman, top-up/tol/rekonsiliasi/penyerahan Flazz, hapus BBM dari Flazz, dan util editor destruktif (`cleanupOrphanPhotos`, `backfillJalurStatus`, `backfillFlazzCardName`, `debugFlazzCard`, `configureSpreadsheet`, `migrateLegacyPasswords` — dijaga token + guard SUPERADMIN).
